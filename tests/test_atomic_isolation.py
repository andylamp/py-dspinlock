...
def test_distributed_spinlock_mutex(
    redis_session: redis.Redis, hashable_string_factory: Callable, spawner_factory: Callable, keygen
):
    ...
    # now delete it
    redis_session.delete(keygen(the_obj))
...
def test_distributed_spinlock_mutex_return_immediately_if_computed(
    redis_session: redis.Redis, hashable_string_factory: Callable, spawner_factory: Callable, caplog, keygen
):
...
def test_spinlock_invalid_mutex_value_encountered(
    ...
    """Tests that the exception for invalid mutex value is thrown correctly."""
    the_obj = hashable_string_factory()
    # compose the key based on the object
    key = keygen(the_obj)
    # set the value to be blocked now, so that the spinlock expires.
