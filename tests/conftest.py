...
async def query_task(task_id: int, obj: str, fail_if_key_exists: bool = False, cached_if_computed: bool = False):
    """
...
async def spawner_factory() -> Callable:
    ...
    async def _spawner(obj_to_use: str, fail_if_key_exists: bool = False, cached_if_computed: bool = False):
        """The spawner for the tasks"""
        return await asyncio.gather(
            query_task(1, obj_to_use, fail_if_key_exists, cached_if_computed),
            query_task(2, obj_to_use, fail_if_key_exists, cached_if_computed),
        )
